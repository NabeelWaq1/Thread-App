...
const Actions = ({post}) => {
	...
	const handleLikeAndUnlike = async () => {
		...
		try {
			...
			if(!data.success) return showToast('Error',data.msg,'error');
	...
	const handleReply = async () => {
		...
		try {
			...
			const updatedPosts = posts.map((p) => {
				if (p._id === post._id) {
					return { ...p, replies: [...p.replies, data.reply] }; // data.reply should be the new reply object returned from the API
				}
				return p;
			});
